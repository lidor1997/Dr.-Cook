...
export const register = catchAsync(async (req: RegisterUserRequestType, res: Response) => {
  ...
  return res
    .status(201)
    .json({ id: userRes._id, name: userRes.name, username: userRes.username, favorites: userRes.favorites, token });
});
...
export const loginByToken = catchAsync(async (req: LoginByTokenRequestType, res: Response) => {
  ...
  return res
    .status(200)
    .json({ id: user._id, username: user.username, name: user.name, favorites: user.favorites, token });
});
